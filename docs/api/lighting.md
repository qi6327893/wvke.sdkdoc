...
简要描述：获取当前键盘背光基础配置，其中亮度和速度都使用 0 到 4 档位。
...
当前 `mode` 使用协议枚举值，支持 `0` 到 `23`，以及 `28`。
常用示例：`0=关闭灯光`、`1=静态模式`、`2=呼吸模式`、`3=光谱循环`、`23=旋转轮盘`、`28=涂鸦模式`。
...
• `mode=1 静态模式`: `0=单色模式`、`1=炫彩模式一`、`2=炫彩模式二`、`3=炫彩模式三`、`4=炫彩模式四`、`5=炫彩模式五`、`6=炫彩模式六`、`7=炫彩模式七`、`8=炫彩模式八`、`9=炫彩模式九`
• `mode=2 呼吸模式`: `0=单色模式`、`1=炫彩模式一`、`2=炫彩模式二`、`3=炫彩模式三`、`4=炫彩模式四`、`5=炫彩模式五`、`6=炫彩模式六`、`7=炫彩模式七`、`8=炫彩模式八`、`9=炫彩模式九`
• `mode=3 光谱循环`: `0=模式一`、`1=模式二`
• `mode=4 正弦光波`: `0=从左至右`、`1=由内而外`、`2=从右至左`
• `mode=5 指尖涟漪1`: `0=炫彩背景光一`、`1=炫彩背景光二`、`2=炫彩背景光三`、`3=炫彩背景光四`、`4=红色背景光`、`5=橙色背景光`、`6=黄色背景光`、`7=绿色背景光`、`8=青色背景光`、`9=蓝色背景光`、`10=紫色背景光`、`11=粉色背景光`、`12=白色背景光`
• `mode=6 指尖涟漪2`: `0=背景光关闭`、`1=背景光打开`
• `mode=7 一触即发1`: `0=炫彩背景光一`、`1=炫彩背景光二`、`2=炫彩背景光三`、`3=炫彩背景光四`、`4=红色背景光`、`5=橙色背景光`、`6=黄色背景光`、`7=绿色背景光`、`8=青色背景光`、`9=蓝色背景光`、`10=紫色背景光`、`11=粉色背景光`、`12=白色背景光`
• `mode=8 一触即发2`: `0=背景光关闭`、`1=背景光打开`
• `mode=9 踏雪无痕`: `0=持续模式一`、`1=持续模式二`、`2=触发模式三`、`3=触发模式四`
• `mode=10 暗流涌动`: `0=扩散模式一`、`1=汇聚模式一`、`2=扩散模式二`、`3=汇聚模式二`
• `mode=11 浮光掠影`: `0=自上而下一`、`1=自下而下一`、`2=自上而下二`、`3=自下而下二`
• `mode=12 流光溢彩`: `0=从左至右一`、`1=从右至左二`、`2=从左至右一`、`3=从右至左二`
• `mode=13 繁星点点`: `0=背景光关闭`、`1=背景光打开`
• `mode=14 变换呼吸`: `0=模式一`、`1=模式二`
• `mode=15 斜风细雨`: `0=背景光关闭`、`1=背景光打开`
• `mode=16 川流不息`: `0=背景光关闭`、`1=背景光打开`
• `mode=17 花开富贵`: `0=背景光关闭`、`1=背景光打开`
• `mode=18 跑马游龙`: `0=背景光关闭`、`1=背景光打开`
• `mode=19 漫天飞雪`: `0=背景光关闭`、`1=背景光打开`
• `mode=20 百花齐放`: `0=模式一`、`1=模式二`
• `mode=21 峰回路转`: `0=顺时针模式一`、`1=逆时针模式一`、`2=顺时针模式二`、`3=逆时针模式二`
• `mode=22 贪吃小蛇`: `0=背景光关闭`、`1=背景光打开`
• `mode=23 旋转轮盘`: `0=顺时针模式一`、`1=逆时针模式一`、`2=顺时针模式二`、`3=逆时针模式二`
• `mode=28 涂鸦模式`: `0=模式一`、`1=模式二`

其中 `luminance` 和 `speed` 都是档位值，范围为 `0` 到 `4`。
一般可按以下理解使用：
...
简要描述：设置键盘背光基础配置，其中亮度和速度都使用 0 到 4 档位。
...
• `mode`: 主模式值，支持 `0` 到 `23`，以及 `28`
• `childMode`: 当前主模式下的子模式序号，从 `0` 开始；关闭灯光和单色灯光可忽略此选项
• `luminance`: 亮度档位，范围 `0` 到 `4`
• `speed`: 速度档位，范围 `0` 到 `4`
...
• `luminance` 仅支持 `0` 到 `4` 的档位值。
• `speed` 仅支持 `0` 到 `4` 的档位值。
...
• 页面销毁或不再需要渲染时，应移除事件监听、清理心跳定时器，并调用 `stopLightingRealtimeReport()` 立即通知设备停止实时上报。
...
• 不再需要实时渲染时，点击 `停止监听`，页面会自动移除监听、清理心跳定时器，并调用 `stopLightingRealtimeReport()` 通知设备停止上报。

## 停止灯光实时上报

ServiceKeyboard.stopLightingRealtimeReport(options)

简要描述：立即通知设备停止自定义灯光实时上报。SDK 会自动查找当前活跃的键盘背光或 Logo 灯实时监听，并发送对应停止协议；如果当前没有本地活跃监听状态，会同时通知键盘背光和 Logo 灯停止，避免设备侧继续上报到自然超时。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| options.target | string | 可选停止目标。支持 `auto`、`keyboardBacklight`、`lightingLogo`、`all`，默认 `auto`。普通业务通常不需要传。 | 否 |

### 返回说明

返回对象字段说明：

• `notified`: 是否已向设备发送停止通知
• `targets`: 本次选择的停止目标
• `packetCount`: 本次发送的停止协议包数量
• `protocolOperation`: 固定为 `3`，对应停止实时上报操作位 `0x03`
