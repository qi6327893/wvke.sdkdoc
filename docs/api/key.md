# 按键功能

## 概述

本章节介绍当前 SDK 中与按键布局和单键配置相关的 API。

## 获取键盘默认按键布局

ServiceKeyboard.getDefaultKeyLayout()

简要描述：获取键盘默认按键布局。

### 参数

此方法不需要参数。

### 返回值

• 总体类型：`Promise<any>`
• 描述：返回默认按键布局数据，具体结构取决于设备布局定义。

### 使用示例

```javascript
const layout = await ServiceKeyboard.getDefaultKeyLayout();
console.log('默认按键布局:', layout);
```

## 获取按键自定义层功能

ServiceKeyboard.getKeyCustomLayer(layer, options?)

简要描述：获取指定层级的按键自定义功能映射，当前支持基本层、FN1、FN2、FN3 层。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| layer | number | 层级值。基本层：`0x00`，FN1 层：`0x01`，FN2 层：`0x02`，FN3 层：`0x03` | 是 |
| options | object | 可选图片资源选项。`includeImages: false` 时 `image` 为空字符串；`verifyImages: true` 时才会主动请求图片以验证可访问性 | 否 |

### 返回值

• 总体类型：`Promise<{ code: number, layer: number, list: Array<{ hid: string, type: string, name: string, content: string, image: string, fn_code?: string }>, layout: any[] }>`
• 描述：返回指定层级的自定义功能列表，以及当前键位布局映射。其中 `image` 为对应自定义功能图标资源地址；若当前项没有可用图标或传入 `{ includeImages: false }`，则返回空字符串。默认只返回 URL，不主动加载图片；只有传入 `{ verifyImages: true }` 时才会请求图片资源做可访问性验证。

### 使用示例

```javascript
const result = await ServiceKeyboard.getKeyCustomLayer(0x00);
console.log('基础层自定义功能:', result);

const resultWithoutImages = await ServiceKeyboard.getKeyCustomLayer(0x00, {
  includeImages: false,
});
```

## 获取自定义快捷键 CODE

ServiceKeyboard.getHotKeyCodeCustom(hotKey)

简要描述：通过自定义快捷键组合获取协议 CODE。该接口为客户端封装功能，支持单个修饰键和多个修饰键组合。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| hotKey | string | 快捷键组合，例如 `Ctrl+C`、`Ctrl+Shift+S`、`Ctrl+Alt+Delete`。修饰键支持 `Ctrl`、`Shift`、`Alt`、`Win` | 是 |

### 返回值

• 总体类型：`Promise<{ code: number, hotKey: string, keyName: string, modifiers: string[], shortcutCode: string, id: number, commandType: number, key: string, modifier: string, config: { id: string, type: string } }>`
• 描述：`shortcutCode` 是可展示的协议 CODE，例如 `0x06,0x61`；`id` 是普通按键 HID 数值；`commandType` 是热键修饰符协议值；`config` 可直接作为 `setKeyConfig` 的第三个参数使用。

### 使用示例

```javascript
const hotKeyCode = await ServiceKeyboard.getHotKeyCodeCustom('Ctrl+Alt+Delete');
console.log('自定义快捷键 CODE:', hotKeyCode.shortcutCode);
await ServiceKeyboard.setKeyConfig('A', '0x00', hotKeyCode.config);
```

## 设置单个按键功能

ServiceKeyboard.setKeyConfig(keyName, layer, config)

简要描述：设置指定按键在指定层级下的功能配置。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| keyName | string | 按键名称，例如 `A`、`B`、`Esc` 或 `Space`。SDK 会根据按键名称自动解析对应 HID | 是 |
| layer | number | 层级值。基本层：`0x00`，FN1 层：`0x01`，FN2 层：`0x02`，FN3 层：`0x03` | 是 |
| config | object | 按键配置对象。`config.id` 为具体功能值，`config.type` 为功能值的修饰符；具体功能描述值请参考协议表 | 是 |

### 返回值

• 总体类型：`Promise<any>`
• 描述：返回设置后的按键配置结果。

### 使用示例

```javascript
const result = await ServiceKeyboard.setKeyConfig('A', 0x00, {
	id: 0x05,
	type: 0x00
});
console.log('设置按键结果:', result);
```

## 获取按键编组

ServiceKeyboard.getKeyGroup()

简要描述：获取整个键盘的按键编组信息（Layout 0xFF）。所有按键默认属于编号 0 组。

### 参数

此方法不需要参数。

### 返回值

• 总体类型：`Promise<{ code: number, total: number, details: Array<{ row: number, col: number, keyName: string, hid: string, group: number }>, layout: Array<Array<...>> }>`
• 描述：`details` 为平铺的按键编组列表，`layout` 为保留键盘行列结构的二维数组。每个按键包含 `group` 字段表示其编组编号。

### 使用示例

```javascript
const groupResult = await ServiceKeyboard.getKeyGroup();
console.log('按键编组:', groupResult);
console.log('编组 0 的按键数:', groupResult.details.filter(k => k.group === 0).length);
```

## 设置按键编组

ServiceKeyboard.setKeyGroup(params)

简要描述：设置指定按键的编组编号（Layout 0xFF）。支持单键和批量模式。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| keyName | string | 按键名称。单键模式与 `group` 配合使用 | 单键时是 |
| hid | string | 按键 HID，如 `0x04`。单键模式可选 | 否 |
| group | number | 编组编号。默认 0 | 否 |
| keys | array | 批量模式：按键数组，每项包含 `keyName` 和可选的 `hid`、`group` | 批量时是 |

### 返回值

• 单键模式：`Promise<{ code: number, keyName: string, hid: string, group: number }>`
• 批量模式：`Promise<{ code: number, batch: true, total: number, details: Array<{ keyName: string, hid: string, group: number }> }>`

### 使用示例

```javascript
// 单键模式
await ServiceKeyboard.setKeyGroup({ keyName: 'A', group: 1 });

// 批量模式
await ServiceKeyboard.setKeyGroup({
  keys: [
    { keyName: 'A', group: 1 },
    { keyName: 'B', group: 1 },
    { keyName: 'C', group: 2 }
  ]
});
```


## 获取 RT 按键编组

ServiceKeyboard.getRtKeyGroup()

简要描述：获取整键盘所有按键的 RT 编组信息。协议功能码 `0xFE`，默认全局所有按键都是编号 0 组。

### 参数

此方法不需要参数。

### 返回值

• 总体类型：`Promise<{ code: number, total: number, details: Array<{ row, col, keyName, hid, group }>, layout: Array<Array<...>> }>`

### 使用示例

```javascript
const result = await ServiceKeyboard.getRtKeyGroup();
console.log('RT 编组总数:', result.total);
```

## 设置 RT 按键编组

ServiceKeyboard.setRtKeyGroup(params)

简要描述：设置按键的 RT 编组编号。支持单键和批量模式。协议功能码 `0xFE`，格式为 `HID + 组号`。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| keyName | string | 按键名称（单键模式） | 否 |
| group | number | RT 编组编号，默认 `0` | 否 |
| keys | `Array<{ keyName?, group? }>` | 批量模式：按键数组 | 否 |

### 返回值

• 单键模式：`Promise<{ code, keyName, hid, group }>`
• 批量模式：`Promise<{ code, batch: true, total, details }>`

### 使用示例

```javascript
// 单键模式
await ServiceKeyboard.setRtKeyGroup({ keyName: 'A', group: 1 });

// 批量模式
await ServiceKeyboard.setRtKeyGroup({
  keys: [
    { keyName: 'A', group: 2 },
    { keyName: 'S', group: 2 },
    { keyName: 'D', group: 2 }
  ]
});
```

## 恢复指定按键功能

ServiceKeyboard.recoverKeyConfig(keyName, layer)

简要说明：恢复指定按键在指定层级下的出厂功能。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| keyName | string | 按键名称，例如 `A`、`B`、`Esc` 或 `Space`。SDK 会根据按键名称自动解析 HID | 是 |
| layer | number | 层级值。基本层：`0x00`，FN1层：`0x01`，FN2层：`0x02`，FN3层：`0x03` | 是 |

### 返回值

- 整体类型：`Promise<{ code: number, keyName: string, hid: string, layer: number, recoverTarget: number }>`
- 描述：返回恢复结果，包含按键名称、HID、层级和恢复对象类型。

### 使用示例

```javascript
// 恢复按键 A 在基本层的功能
const result = await ServiceKeyboard.recoverKeyConfig('A', 0x00);
console.log('恢复结果:', result);

// 恢复按键 A 在 FN1 层的功能
await ServiceKeyboard.recoverKeyConfig('A', 0x01);
```

## 恢复全部按键功能

ServiceKeyboard.recoverAllKeyConfig(layer)

简要说明：恢复指定层级下所有按键的出厂功能。

### 参数

| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| layer | number | 层级值。基本层：`0x00`，FN1层：`0x01`，FN2层：`0x02`，FN3层：`0x03` | 是 |

### 返回值

- 整体类型：`Promise<{ code: number, layer: number, recoverTarget: number }>`
- 描述：返回恢复结果，包含层级和恢复对象类型。

### 使用示例

```javascript
// 恢复基本层所有按键的出厂功能
const result = await ServiceKeyboard.recoverAllKeyConfig(0x00);
console.log('恢复全部结果:', result);

// 恢复 FN1 层所有按键的出厂功能
await ServiceKeyboard.recoverAllKeyConfig(0x01);
```