...
| options | object | 可选图片资源选项。`includeImages: false` 时不返回 SDK 图片地址；`verifyImages: true` 时才会主动请求图片以验证可访问性 | 否 |
...
- `image` 为功能图标资源地址；无图标或传入 `{ includeImages: false }` 时为空字符串。默认只返回 URL，不主动加载图片；只有传入 `{ verifyImages: true }` 时才会请求图片资源做可访问性验证。
...
| 字段 | 类型 | 描述 | 是否必需 |
|------|------|------|----------|
| options | object | 可选图片资源选项。`includeImages: false` 时 `images` 内字段均为空字符串；`verifyImages: true` 时才会主动请求图片以验证可访问性 | 否 |
...
- `images` 返回轴体图标和结构图片资源地址。默认只返回 URL，不主动加载图片；传入 `{ includeImages: false }` 时 `icon` / `z_b` / `z_y` 均为空字符串；只有传入 `{ verifyImages: true }` 时才会请求图片资源做可访问性验证。
...
- 不使用 SDK 内置图片时，建议给图标相关接口传 `{ includeImages: false }`，避免业务侧处理无用图片地址；需要验证静态资源部署时再传 `{ verifyImages: true }`。
