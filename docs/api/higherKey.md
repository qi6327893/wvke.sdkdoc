...
| 字段 | 含义 |
|------|------|
| mode | SOCD 模式值，`0` 后输入优先，`1` 绝对优先(keyName)，`2` 绝对优先(keyName2)，`3` 抵消模式，`4` 深度优先 |
| delay | SOCD 延迟值 |
...
| mode | number \| string | SOCD 工作模式，支持 `0/1/2/3/4` 或 `socdLaterInputPriority/socdAbsolutePriority/socdAbsolutePriority2/socdNeutral/socdCancel/socdDepthPriority` | 是 |
