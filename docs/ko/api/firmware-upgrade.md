...
`ServiceFirmwareUpgrade.connectBootDevice(options?)` opens the WebHID picker, opens the selected BOOT device, and stores it for a later object-format `upgrade()` call.

### Parameters

| Field | Type | Description | Required |
|------|------|-------------|----------|
| options.filters | Array | WebHID filter array | No |
...
`upgrade({ binFilePath?, binData?, onProgress, ... })` uses the BOOT device saved by `connectBootDevice()`.

| Field | Type | Description | Required |
|------|------|-------------|----------|
| binFilePath | string \| Blob | Firmware `.bin` path, URL, or browser file object; mutually exclusive with `binData` | No |
