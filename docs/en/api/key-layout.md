...
â€?`total`: total number of supported backlight main modes
â€?`list`: supported backlight effect list

Each item in `list` contains:

â€?`mode`: backlight main mode value
â€?`label`: backlight main mode label
â€?`childModes`: supported sub-mode list for the current main mode

Each item in `childModes` contains:

â€?`value`: sub-mode value, starting from `0`
â€?`label`: sub-mode label
...
â€?`total`: total number of supported ambient-light main modes
â€?`list`: supported ambient-light effect list

Each item in `list` contains:

â€?`mode`: ambient-light main mode value
â€?`label`: ambient-light main mode label
â€?`alias`: protocol alias, such as `L0` through `L9`
...
ServiceKeyboard.getShortcutFunctionList(type, options?)
...
- `image` is the icon URL, or an empty string when no icon exists or `{ includeImages: false }` is passed. By default, the SDK only returns URLs and does not proactively load images; it requests image assets only when `{ verifyImages: true }` is passed.
...
const mediaShortcutsWithoutImages = await ServiceKeyboard.getShortcutFunctionList('media', {
	includeImages: false,
});
```
...
ServiceKeyboard.getFineTuneAxisList(options?)
...
| Field | Type | Description | Required |
|------|------|------|----------|
| options | object | Optional image asset options. Use `includeImages: false` to return empty `images` fields; use `verifyImages: true` only when the SDK should request image assets to verify accessibility | No |
...
- `images` contains switch icon and structure image URLs. By default, the SDK only returns URLs and does not proactively load images; with `{ includeImages: false }`, `icon`, `z_b`, and `z_y` are empty strings; it requests image assets only when `{ verifyImages: true }` is passed.
...
const fineTuneAxisListWithoutImages = await ServiceKeyboard.getFineTuneAxisList({
	includeImages: false,
});
```
...
- If your application does not use SDK-provided images, pass `{ includeImages: false }` to image-related APIs; pass `{ verifyImages: true }` only when validating static asset deployment.
