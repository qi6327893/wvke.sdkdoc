...
- `firmwareUpgrade.upgrade(device, binFilePath, options?)`
  - Also supports object-format: `upgrade({ binFilePath?, binData?, onProgress, ... })`
...
Brief: Send the enter-BOOT command to an opened app-mode device. The device reboots automatically; after the reboot, call `connectBootDevice()` or `navigator.hid.requestDevice()` again to select and open the BOOT device.
...
Brief: Open the browser WebHID device picker to select and open a BOOT-mode device. The SDK saves the selected device, so subsequent `upgrade()` calls can omit the device parameter.

### Parameters

| Field | Type | Description | Required |
|------|------|-------------|----------|
| options.filters | Array | WebHID filter array, defaults to built-in VID/PID list | No |

### Return Value

Overall type: `Promise<{ code: number, success: boolean, message: string, device: object }>`
...
The method also accepts object-format: `upgrade({ binFilePath?, binData?, onProgress, ... })`, which uses the BOOT device saved by `connectBootDevice()`.

### Parameters (traditional format)

| Field | Type | Description | Required |
|------|------|-------------|----------|
| device | HIDDevice | An opened BOOT-mode WebHID device | Yes |
| binFilePath | string \| Blob | Firmware `.bin` path or browser file object | Yes |
...
| Field | Type | Description | Required |
|------|------|-------------|----------|
| binFilePath | string | `.bin` firmware path or HTTPS URL, mutually exclusive with `binData` | No |
| binData | ArrayBuffer \| Uint8Array | Pre-loaded firmware binary data, mutually exclusive with `binFilePath` | No |
| onProgress | `(progress) => void` | Upgrade progress callback | No |
| timeout | number | Normal command response timeout. Default: `5000` ms | No |
| eraseTimeout | number | Erase phase timeout. Default: `15000` ms | No |
| writePacketDelay | number | Delay between firmware packet groups. Default: `2` ms | No |
| commandDelay | number | Delay between signing chain commands. Default: `20` ms | No |
...
Overall type: `Promise<{ code: number, success: boolean, source?: string | Blob, size: number, paddedSize: number, packetCount: number, crc: number, deviceCrc: number }>`

| Field | Type | Description |
|------|------|-------------|
| code | number | `0` means the upgrade flow completed |
| success | boolean | Whether the upgrade succeeded |
| source | string \| Blob (optional) | Firmware source; may be absent when `binData` is used |
