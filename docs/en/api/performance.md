...
| rtSwitch | number | RT switch mode: `0`=off, `1`=global(0x20), `2`=on when RT switch OFF(0x30), `3`=on when RT switch ON(0x40) | No |
...
Brief description: Returns the axis (switch) configuration for the entire keyboard (Layout 19), including each key's current axis assignment.
...
- Description: `details` and `list` are flat arrays of axis config entries; `layout` preserves the keyboard structure. Each key includes `axis` (switch ID), `brand`, `model`, `axisRangeMax`, `axisCoefficient`, and `supportsFineTune`.

### Example

```javascript
const axisDetails = await ServiceKeyboard.getAxisDetails();
console.log('Axis details:', axisDetails);
console.log('Keys with TTC switches:', axisDetails.details.filter(k => k.brand === 'TTC').length);
```
...
Brief description: Assigns a target axis (switch) to specified keys (Layout 19). Supports single-key and batch modes with optional range and coefficient values.

### Parameters

| Field | Type | Description | Required |
|------|------|------|----------|
| keyName | string | Key name. Used with `axis` in single-key mode | Yes (single) |
| axis | number | Axis (switch) index. Default 24 | No |
| axisRangeMax | number | Maximum travel of the axis, optional | No |
| axisCoefficient | number | Axis coefficient, optional | No |
| keys | array | Batch mode: array of key objects, each can override `axis`, `axisRangeMax`, `axisCoefficient` | Yes (batch) |

### Returns

- Single-key: `Promise<{ code: number, keyName: string, hid: string, axis: number }>`
- Batch: `Promise<{ code: number, batch: true, total: number, details: Array<{ keyName: string, hid: string, axis: number }> }>`

### Example

```javascript
// Single key
await ServiceKeyboard.setKeyCustomAxis({ keyName: 'A', axis: 24 });
