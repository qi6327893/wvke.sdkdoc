...
- WebHID authorization, authorized-device listing, and selected-device initialization
- RT full-travel switch, actuation travel, and safe-zone tuning
...
- RT travel, advanced-key APIs, and selected-device initialization
...
// On first connection, open the browser authorization picker.
// configs can temporarily override the valid VID/PID filter array.
...
if (code !== 0 || devices.length === 0) {
	throw new Error('No keyboard devices authorized');
}

// Authorized devices can be listed later without reopening the picker.
const authorizedResult = await keyboard.getDevices();
console.log('Authorized devices:', authorizedResult.devices);
